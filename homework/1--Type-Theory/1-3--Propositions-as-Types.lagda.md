...
filter P L [] = []
filter P L (x :: list) = if L x then x :: list else list
```
...
```
isEvenType : ℕ → Type
isEvenType n = Bool→Type (isEven n)

--twoisEven : isEvenType (suc (suc zero))
--twoisEven = {!   !}
```
...
-- Exercise:
and→Type true b = (λ p → tt , p) , (λ q → snd q)        ---(λ p → tt , p ) ,  snd
...
-- Exercise:
⇒→Type true b = (λ p → λ _ → p) , ev tt ---(const , flip apply tt)/evaluate tt
⇒→Type false b = (λ p → ∅-rec) , λ q → tt  ---(λ _ → ∅-rec) , const tt
...
not→Type true = const , ev tt
not→Type false = (λ p → λ x → x) , const tt
```
...
-- Exercise
implies¬¬ P = λ p → λ np → np p


¬¬¬implies¬ : (P : Type) → (¬ ¬ ¬ P) impliesP (¬ P)
-- Exercise
¬¬¬implies¬ P nnnp = λ p → nnnp (λ np → np p)   
```
...
-- Exercise:
zeroImpliesEven zero = λ z → tt
zeroImpliesEven (suc n) = ∅-rec -- if left side is false then the implication is true
```
