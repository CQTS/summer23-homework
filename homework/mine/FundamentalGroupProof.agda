...
module homework.mine.FundamentalGroupProof where
...
substPathsR' {y = y} x p = funExt homotopy where
  ...
  homotopy q = J motive base-case p
    where
      motive : ∀ z' → y ≡ z' → Type _
      motive z' p' = subst (_≡_ x) p q ≡ q ∙ p

      base-case : motive y refl
      base-case = {!   !}
...
substPathsR : {C : Type ℓ}{y z : C} → (x : C) → (p : y ≡ z) → subst (λ y → x ≡ y) p ≡ λ q → {!q ∙ p!}
substPathsR {y = y} x p = funExt homotopy where                 -- B        (y ≡ z)       B y → B z <=>   x ≡ y → x ≡ z  path between (x≡z) ≡ 
  homotopy : ∀ q → subst (λ y → x ≡ y) p q ≡ q ∙ p  -- subst (λ y → x ≡ y)    p     <->   B y -- x ≡ y  <->   B z  ( x ≡ z )
